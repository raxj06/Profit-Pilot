...
const BillDetails = ({ bill: initialBill, onBack, user }) => {
  ...
  // Function to delete the current bill
  const handleDelete = async () => {
    const billName = bill.invoice_number || `INV-${bill.id?.slice(-8)}`;
    ...
    try {
      setDeleting(true);
      console.log('🗑️ Deleting bill:', bill.id);

      const backendUrl = import.meta.env.VITE_BACKEND_URL || '';
      const { data: { session } } = await supabase.auth.getSession();

      const response = await fetch(
        `${backendUrl}/api/bills/${bill.id}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-red-700 bg-red-50 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50"
          >
            {deleting ? (
              <>
